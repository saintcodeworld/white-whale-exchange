...
function pickRandomSpin(): { index: number; value: number; rarity: string } {
  ...
  for (let i = 0; i < SEGMENTS.length; i++) {
    ...
    if (rand <= 0) return { index: i, value: SEGMENTS[i].value, rarity: SEGMENTS[i].rarity };
  }
  return { index: 0, value: SEGMENTS[0].value, rarity: SEGMENTS[0].rarity };
}
...
export default function SpinWheel() {
  ...
  const handleCollect = useCallback(() => {
    ...
    if (!user) {
      ...
      // Logged in — just dismiss the result modal
      setShowResult(false);
    }
  }, [result, user]);
  ...
  return (
    <div className="spin-wheel-container">
      ...
      <WithdrawModal
        ...
        onWithdraw={(amount: number, wallet: string) => {
          const success = handleWithdraw(amount, wallet);
          if (success) {
